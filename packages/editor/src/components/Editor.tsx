...
import { isAtBlockStart } from "../editing/block-navigation.js";
...
export function Editor({ value, onChange, className }: EditorProps) {
  ...
  const handleBackspaceKey = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      const newText = value.slice(0, clampedOffset - 1) + value.slice(clampedOffset);
  ...
  const handleEnterKey = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      const newText = value.slice(0, clampedOffset) + insertion + value.slice(clampedOffset);
